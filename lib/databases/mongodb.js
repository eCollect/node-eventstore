...
const resolvify = (callback) => callback ? callback.bind(null, null) : null;
const rejectify = (callback) => (err) => {
		debug(err);
...
async function streamEventsByRevision(self, findStatement, revMin, revMax, resultStream, lastEvent) {
  ...
  var mongoStream = self.events.find(findStatement, {
		sort: [['commitStamp', 'asc'], ['streamRevision', 'asc'], ['commitSequence', 'asc']]
	}).stream();
...
_.extend(Mongo.prototype, {
  ...
  getEventsSince: function (date, skip, limit, callback) {
    ...
    const res = await this.events.find(findStatement, {
			sort: [['commitStamp', 'asc'], ['streamRevision', 'asc'], ['commitSequence', 'asc']]
		}).toArray();
  ...
  getUndispatchedEvents: function (query, callback) {
    ...
    this.events.find(findStatement, { sort: [['commitStamp', 'asc'], ['streamRevision', 'asc'], ['commitSequence', 'asc']] })
			.toArray().then(resolvify(callback)).catch(callback);
  },
  ...
  getLastEvent: function (query, callback) {
    ...
    this.events.findOne(findStatement, {
			sort: [['commitStamp', 'desc'], ['streamRevision', 'desc'], ['commitSequence', 'desc']]
		}).then(resolvify(callback)).catch(callback);
  },
